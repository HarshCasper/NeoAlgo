...
/*
First we can solve this problem thorough a Naive Solution , that is a Brute force Solution which will be O(N * N)
*/
...
bool Pair_exist_by_Brute_force(int ar[], int N, int X)
{
    for(int i = 0; i < N; i++)
    {
        for(int j = 0; j < N; j++)
...
int main()
{
    ...
    bool solve = Pair_exist_by_Brute_force(ar , N , X);
    if(solve)
...
/*
Secondly , we can optimize our solution by using Two pointers Method and thus time complexity will be reduced to O(N)
