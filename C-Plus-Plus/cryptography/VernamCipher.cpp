...
string decrypt(string enc,string key){
	...
	for(int i=0;i<n;i++){
		if(enc[i]>=key[i]){
			...
			dec+=char(enc[i]-key[i]+91);  // enc[i]-key[i]+26+65
		}
	}
	return dec;
...
int main(){
	...
	cout<<"Enter the Key: ";
	cin>>key;
  transform(key.begin(), key.end(), key.begin(), ::toupper); // convert to uppercase if not.
	
	cout<<"\nEnter the plain text of same size as that of key: ";
	cin>>plain;
  transform(plain.begin(), plain.end(), plain.begin(), ::toupper);  // convert to uppercase if not.
