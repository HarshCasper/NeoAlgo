...
#include <vector>
using namespace std;
int count_touches(int mask, vector<string>& arr, int n, int N, int pos = 0)
{
	...
	int a = cnt + count_touches(mask1, arr, n, N, pos + 1) + count_touches(mask2, arr, n, N, pos + 1);
	int b = count_touches(mask, arr, n, N, pos + 1);
...
int minimumTouchesRequired(int N, vector<string> arr) {
	...
	return count_touches((1 << N) - 1, arr, n, N);
}
