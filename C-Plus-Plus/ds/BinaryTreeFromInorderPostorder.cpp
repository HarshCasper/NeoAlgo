...
TreeNode * helper(vector<int>inorder,vector<int>postorder,int Instart,int InEnd,int Poststart,int PostEnd){
	...
	for(rootIndex=Instart ; rootIndex<=InEnd ; rootIndex++){
		if(inorder[rootIndex]==rootVal) //We have completely traversed left Subtree
		break;
	}
	int leftTree=rootIndex-Instart; //length of left Tree
	int rightTree=InEnd-rootIndex;//Length of right Tree
...
int main(){
	...
	//buildTree function returns root of the newly formed Tree
	TreeNode *root=buildTree(inorder , postorder);
	//checking the inorder traversal of this newly Build Binary Tree with given Inorder Traversal
	cout<<"The Preoder Traversal of the Tree is : ";
	preoderTraversal(root);
	for(int i=0;i<preoderRes.size();i++)
	cout<<preoderRes[i]<<" ";
