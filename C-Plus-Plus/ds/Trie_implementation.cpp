//Tries are tree based structures used for efficient retrieval of key form huge set of strings
//This is a basic implementation of Trie in which strings are inserted and then there are some queries to find if strings are present in trie or not
//Time Complexity for insert and find function: O(l) where l is the length of word 
//Space complexity: O(n*m) where n is number of keys and m=max(l) i.e max length of input strings
