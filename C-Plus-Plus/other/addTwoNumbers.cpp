...
node* AddTwoNumbers(node* list1,node* list2){
    node* answer_list = NULL; // initialize a list to store the result.
    node *pointer1 = list1;         // Two temporary pointers of 'type' node pointing to list 1 and 2 respectively.
    node *pointer2 = list2;
    int carry = 0; // to store the carry obtain if the sum exceeds 9.
    while (pointer1 != NULL || pointer2 != NULL)
    {
        int x = (pointer1 != NULL) ? pointer1->value : 0; // a temporary variable 'x' to store the value at the current node of list1
        int y = (pointer2 != NULL) ? pointer2->value : 0; // a temporary variable 'y' to store the value at the current node of list2
        int sum = carry + x + y; // calculating sum of the respective nodes
        carry = sum / 10; // calculating the carry
        Insertion(answer_list,(sum % 10)); // storing the sum obtained in the answerList;
        if (pointer1 != NULL)
...
int main()
{
    ...
    while (testCases--)
    {
        node* input1 = NULL; // intialization of the input list1 
        node* input2 = NULL; // intialintialization of the input list2
        int number1;
