...
int main(){
	...
	cin>>s1>>s2;
	//n and k denotes size of string s1 and s2 respectively
	int n=s1.length();
	int k=s2.length();
	//declaring a variable to keep count of anagrams and initialising it from zero
	int ans=0;
	...
	for(int i=0;i<k;i++)
		...
		mp[s2[i]]++;
	//declaring a variable to store size of map
	int count=mp.size();
	...
	while(j<n){
			//decreasing the frequency count of element
			mp[s1[j]]--;
		...
		if(j-i+1<k)
			j++;
		//on hitting size of window if count reaches 0 then its anagram
		//store it in ans
		else if(j-i+1==k)
		{//size of map hits zero
			if(count==0)
			//increase the count of anagram
				ans++;
			...
			//increase the count if element found again
			if(mp[s1[i]]==1)
				count++;
			//slide the window
			i++;
