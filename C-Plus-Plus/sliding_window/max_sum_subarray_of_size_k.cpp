/*
Problem statement :
An array of integers of size 'n' and a number 'k' is given. We have to detemine the maximum sum of subarray of size k
=> Input format:
   size of array (n)
   n elemnts of array
   k
...
int max_sum_subarray(int n, int arr[], int k)
{
    int sum = 0;
    int maxx = INT_MIN;
    /*two pointers are needed to represent start and end of the window.let's denote i as the start and j as the end then window size (k) =j-i+1*/
    int i = 0, j = 0;
