...
int main()
{
  int n; //Declarartion of required variables
  ...
  for (int i = 0; i < n - 1; i++)
  {
    ...
    for (int j = 0; j < n - 1; j++)
    {
      ...
      if (arr[j] > arr[j + 1])
      {
        //Checking if the element if larger than the next element
    ...
    if (sorted == 0)
    {
      // If the elements aren't swapped and int sorted = 0 only then array is already sorted
      break;
    }
  }
