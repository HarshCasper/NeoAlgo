...
typedef struct stack {
    int capacity, top;
    char *arr;
}stack;     /* creating a structure to implement stack */
...
void reverseWords(char sentence[100], int length) {
    ...
    for ( int  i = 0; i < length; i ++){
        ...
        if ( (! isspace(letter)) && (letter != '\n') && (letter != '\0')) {
            push(s, letter);    /* anything other than an alphabet or number indicates that a word has ended */
        }
        else {
            while ( !(isEmpty(s)) ) {
           printf("%c", pop(s));   
...
int main() {
    ...
    reverseWords(sentence, strlen(sentence));    /* calling the reverseWords function */
    return 0;
}
