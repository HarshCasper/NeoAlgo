...
Hence, the height of an AVL tree is always O(log N), where N is the number of nodes in the tree.

O(1)/constant time: Rotate Operation, Update height and adjust balance factor.
O(log N)/O(h) time: Insertion and Deletion operations, where h is the height of the tree.
O(N) time: Tree Traversal Operations (Pre, Post and Inorder).
O(N) space complexity: AVL tree operations take O(N) space (stack size is included), where N is the number of nodes in the tree.
