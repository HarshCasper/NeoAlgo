...
# A square consists of a set of 4 points that form the vertices of a square. Each side of the 
...
def Counting_squares(r, c):
    ...
    # parallel to the axises the general formula for the number of squares parallel to the x-and y-axis
    # in a n x m  square of points to be equal to ∑ (n−k)(m-k) k is from 1 to n-1
    # where k represents the subproblem of size k i.e square of side k 
    add = (r - k) * (c - k) 
    while add > 0:
        # since we need to include all the squares including those which are inclined.we observe
        # that for every square of side length = k there are k inclined squares
        # therefore we multiply our current variable add with k
        ans += add * k
        k += 1
        add = (r-k) * (c-k) 
    return ans % (10 ** 9 + 7)
