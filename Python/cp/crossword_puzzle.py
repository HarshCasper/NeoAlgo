...
4. If the word cannot be filled, Backtrack the previous word and fill it in some other position
    ...
    for i0 in range(10):
        for j0 in range(10):
            if crossword0[i0][j0] in ('-', words0[index0][0]):
                ...
                inserted=insertVerticalIfPossible(i0, j0, crossword0, words0, index0, wordfill0)
                if inserted:
                ...
                inserted = insertHorizontalIfPossible(i0, j0, crossword0, words0, index0, wordfill0)
                if inserted:
                    solveCrossword(crossword0, words0, index0 + 1, wordfill0)
                if inserted:
...
def removeVertically(i4, j4, crossword4, words4, index4, wordfill4):
    for x4 in enumerate(words[index4]):
