...
### Purpose
# To write the driver code that takes input and a function that finds the intersection of given two linked lists

### Method
# Calculate size of both given linked lists by counting no.of nodes and find difference between them , say d.
...
# Input are taken as two lists , contaning the data of each node of a Linked List inorder seperated by space 
...
def size(head):
    # Initialise a int "size" with 0 value
    size= 0
    # Traverse all nodes till u reach tail
    while head:
        # Every time u visit a node , increment "size" and move head pointer forward
        size += 1
        head = head.next
    return size
...
def eq(head1,head2):
    ...
    if(head1==None or head2==None):
        return False 
    # If data's of both nodes are equal , return if their successive nodes are equal recursively
...
if __name__ == '__main__':
    ...
    # Take input of values seperated by space and split them to form a list to store data values of Linked List 1 in order
    ...
    if ans:
        print("The intersection point is", ans.data)    # If node is not NONE , print data of the node
    else:
        print("The lists do not intersect.")            # If node is NONE , print that the given lists do not intersect
