...
Motivation: https://www.youtube.com/watch?v=4WRANhDiSHM&list=TLPQMjcxMjIwMjCzl4040phelQ&index=2
...
def bwt(orignal_string):
    ...
    # Roatating orignal string letter by letter and storing the cyclic rotations
...
def invert_bwt(bwt_string):
    ...
    # Store length of bwt_string
    length = len(bwt_string)
    ...
    # Roatating bwt_string letter by letter and storing the cyclic rotations
    for i in range(length):
        bwt_rotations = sorted(
            bwt_string[i] + bwt_rotations[i] for i in range(length))
...
if __name__ == '__main__':
    ...
    b = bwt(input())
    print("BWT compressed string: ", b.replace("$", ""))
    print("Inverted bwt to get orignal string: ", invert_bwt(b).strip("$"))
