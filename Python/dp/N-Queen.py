...
class QueenChessBoard:
    ...
    def is_this_column_safe_in_next_row(self, column):
        '''Checks if the current column is safe in the next row.'''
        ...
        for queen_row, queen_column in enumerate(self.columns):
            s = self.size - queen_column
            if (s - queen_row == (self.size - column) - row):
                return False

        return True

    def display(self):
        '''Displays the current chessboard before printing it.'''
        self.count += 1
...
def solve_queen(size):
    '''Displays a N×N Chessboard with all possibilities of N Queens.'''
