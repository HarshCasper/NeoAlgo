...
class Strassen_Matrix(object):
    ...
    def StrassenMethod(self, x, v):
        ...
        if len(x) == 1:
            return x*v
        # Splitting matrix x and v into  quadrants
        a, b, c, d = self.SplitMat(x)
        ...
        ij22 = P1+P5-P3-P7
        self.C = num.vstack(
            (num.hstack((ij11, ij12)), num.hstack((ij21, ij22))))
...
if __name__ == "__main__":
    ...
    mat_A = np.array(ele_A).reshape(2, 2)
    mat_B = np.array(ele_B).reshape(2, 2)

    rs = Strassen_Matrix(mat_A, mat_B)
    print(rs.MatMul())
